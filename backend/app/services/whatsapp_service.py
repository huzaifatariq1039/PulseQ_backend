...
def send_queue_message(phone: str, name: str, position: int, wait_time: int,
                       doctor_name: str = "N/A", hospital_name: str = "PulseQ Clinic",
    ...
    try:
        ...
        if template_name == "token_number":
            if TWILIO_TOKEN_NUMBER_SID:
                ...
                body = (
                    f"Apki appointment book ho chuki h!\n\n"
                    f"Doctor: {_template_param(params, 0, 'Doctor')}\n"
                    f"Patient: {_template_param(params, 1, 'Patient')}\n"
        ...
        if template_name == "final_alert":
            if TWILIO_FINAL_ALERT_SID:
                ...
                body = (
                    f"Hello {params[0]},\n\nAapki turn kisi bhi waqt aa sakti hai. "
                    f"Please hospital ki taraf rawana ho jayein.\n\n"
                    f"Aapka token number {params[1]} hai.\n\nPulseQ"
                )
                return client.messages.create(from_=from_number, to=formatted_phone, body=body).sid
