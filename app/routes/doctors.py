...
async def receptionist_update_doctor(
    ...
    persist.pop("room", None)
    persist.pop("qualifications", None)
    persist.pop("qualification", None)
    persist.pop("degrees", None)
    persist.pop("experience_years", None)
    persist.pop("phone", None)
    persist.pop("email", None)
...
# ============================================================================
# DEPARTMENT MANAGEMENT (ALL /departments routes BEFORE /{doctor_id})
# ============================================================================
...
# ============================================================================
# SPECIFIC DOCTOR ROUTES (all before generic /{doctor_id})
# ============================================================================
...
async def get_doctor_details(
    ...
    hospital = db.query(Hospital).filter(Hospital.id == doctor.hospital_id).first() if doctor.hospital_id else None
...
async def get_doctor_queue_status(doctor_id: str, db: Session = Depends(get_db)):
    ...
    return QueueStatus(
        ...
        estimated_wait_time=pending_tokens * 3,
